...
void            send_command(char *command, int input_len,
                             char *input_buffer);
...
void init_train_settings()
{
    wm_print(window_id, "Initialize the system.");
...
{
    char            input;
    wm_print(window_id, "Check layout...");
...
int check_zam_dir()
{
    ...
    int             i;
    char            input;
    int             time67;     // the times takes for Zamboni to travel
    // from contact 6 to 4 
    int             time74;     // the times takes for Zamboni to travel
    // from contact 7 to 4 

    wm_print(window_id, "Check if zamboni is there");
    ...
    for (i = 0; found == FALSE && i < 25; i++) {
        ...
        if (input == '1') {
            found = TRUE;
            wm_print(window_id, "Found\n");
        }
        sleep(30);
    }

    if (found == FALSE) {
        wm_print(window_id, "Not found\n");
        return 0;
    }
    // check zamboni's direction
    wm_print(window_id, "Check zamboni direction......");
...
void run_config12()
{
    ...
    wm_print(window_id, "I am back safely!\n\n");
}
...
void train_process(PROCESS self, PARAM param)
{
    ...
    case 120:
        wm_print(window_id,
                 "It is configuration 1 or 2 without Zamboni.\n");
        wm_print(window_id, "I will move soon.\n");
        run_config12();
        break;
    case 340:
        wm_print(window_id,
                 "It is configuration 3 or 4 without Zamboni.\n");
        wm_print(window_id, "I will move soon.\n");
        run_config34();
        break;
    case 121:
        wm_print(window_id, "It is configuration 1 with Zamboni.\n");
        wm_print(window_id, "I will move soon.\n");
        run_config1z();
        break;
    case 122:
        wm_print(window_id, "It is configuration 2 with Zamboni.\n");
        wm_print(window_id, "I will move soon.\n");
        run_config2z();
        break;
    case 341:
        wm_print(window_id, "It is configuration 3 with Zamboni.\n");
        wm_print(window_id, "I will move soon.\n");
        run_config3z();
        break;
    case 342:
        wm_print(window_id, "It is configuration 4 with Zamboni.\n");
        wm_print(window_id, "I will move soon.\n");
        run_config4z();
        break;
    }
