...
#define FRAME_COLOR 0x3f

#define MAGIC_VGA_WINDOW 0x39a2b0f4

typedef struct _VGAWindow {
  int magic;
  const char* title;
  int x, y;
  int width, height;
  struct _VGAWindow* next;
} VGAWindow;

static VGAWindow* root_window = NULL;

static VGAWindow* current_window = NULL;
...
#define	pokeb(S,O,V)		*(unsigned char *)(16uL * (S) + (O)) = (V)
#define	pokew(S,O,V)		*(unsigned short *)(16uL * (S) + (O)) = (V)
#define	_vmemwr(DS,DO,S,N)	k_memcpy((char *)((DS) * 16 + (DO)), S, N)
...
inline static int check_for_clip(unsigned* x, unsigned* y)
{
  if (current_window == NULL) {
    return 0;
  }
  if (*x < current_window->width && *y < current_window->height) {
    *x += current_window->x;
    *y += current_window->y;
    return 0;
  }
...
static void draw_text(const char* txt, int x, int y, int fg_color, int bg_color)
{
  while (*txt != '\0') {
    int i = *txt * 8;
    for (int yy = 0; yy < 8; yy++) {
      int row = g_8x8_font[i++];
      for (int xx = 0; xx < 8; xx++) {
	g_write_pixel(x + xx, y + yy, (row & 0x80) ? fg_color : bg_color);
	row <<= 1;
      }
    }
    x += 8;
...
static void draw_rect(int x, int y, int width, int height, int color)
{
  for (int yy = y; yy < y + height; yy++) {
    for (int xx = x; xx < x + width; xx++) {
      g_write_pixel(xx, yy, color);
    }
  }
}

static void draw_window_frame(VGAWindow* wnd)
{
  current_window = NULL;
  
  int x = wnd->x - 1;
  int h = FONT_HEIGHT + PADDING_TOP + PADDING_BOTTOM;
  int y = wnd->y - h;
  int w = wnd->width + 3;
  draw_rect(x, y, w, h, FRAME_COLOR);
  y++;
  x += (w - k_strlen(wnd->title) * FONT_WIDTH) / 2;
  draw_text(wnd->title, x, y, 0, FRAME_COLOR);
  x = wnd->x - 1;
  y = wnd->y;
  int x2 = x;
  int y2 = y + wnd->height;
  draw_line(x, y, x2, y2, FRAME_COLOR);
  x = x2;
  y = y2;
  x2 = wnd->x + wnd->width + 1;
  draw_line(x, y, x2, y2, FRAME_COLOR);
  x = x2;
  y = y2;
  y2 = wnd->y;
...
static unsigned int create_window(const char* title, int x, int y, int width, int height)
{
  VGAWindow* w = (VGAWindow*) malloc(sizeof(VGAWindow));
  w->magic = MAGIC_VGA_WINDOW;
  w->title = title;
  w->x = x;
  w->y = y;
  w->width = width;
...
void vga_process(PROCESS self, PARAM param)
{
  PROCESS sender;
  VGA_WINDOW_MSG* msg;
  ...
  while (1) {
    ...
    case VGA_CREATE_WINDOW:
      {
	PARAM_VGA_CREATE_WINDOW* param = (PARAM_VGA_CREATE_WINDOW*) &msg->u;
	param->window_id = create_window(param->title,
					 param->x,
					 param->y,
					 param->width,
					 param->height);
    ...
    case VGA_DRAW_TEXT:
      {
	PARAM_VGA_DRAW_TEXT* param = (PARAM_VGA_DRAW_TEXT*) &msg->u;
	current_window = (VGAWindow*) param->window_id;
	draw_text(param->text, param->x, param->y, param->fg_color, param->bg_color);
	break;
      }
    case VGA_DRAW_PIXEL:
      {
	PARAM_VGA_DRAW_PIXEL* param = (PARAM_VGA_DRAW_PIXEL*) &msg->u;
	current_window = (VGAWindow*) param->window_id;
	g_write_pixel(param->x, param->y, param->color);
	break;
      }
    case VGA_DRAW_LINE:
      {
	PARAM_VGA_DRAW_LINE* param = (PARAM_VGA_DRAW_LINE*) &msg->u;
	current_window = (VGAWindow*) param->window_id;
	draw_line(param->x0, param->y0, param->x1, param->y1, param->color);
	break;
      }
    }
    reply(sender);
...
/* TOS_IFDEF vga */  
  ...
  vga_port = create_process(vga_process, 5, 0, "VGA Process");
